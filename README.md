...
本项目用于 2025 年智能救援比赛。机器人需要识别场地中的红、蓝、黄、黑四类物料以及红、蓝安全区，并根据队伍颜色和任务状态完成目标选择、抓取校验和安全区定位。决赛会随机更换物料外形，项目最终获省一等奖。

我负责视觉部分：

- 采集并标注 350 张图像，按 300/50 划分训练集和验证集，使用 Albumentations 将 300 张训练图像增强至 900 张。
- 训练六类 YOLOv8 检测模型，完成 INT8 量化和 MaixCAM Pro 端侧部署。
- 编写目标筛选、策略切换、视觉状态机和 UART 通信程序。

最终，训练阶段准确率为 `99%`，量化模型在数据集上的识别准确率为 `96%`，实际运行链路平均达到 `29.98 FPS`；更换不同形状的物料后仍能完成识别。
...
- 仓库只包含摄像头、目标检测、视觉决策和 UART 输出，不包含底盘控制、机械抓取和无线通信。
