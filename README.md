...
- 构建 350 张原始图像数据集，按 300/50 划分训练集和验证集；使用 Albumentations 对训练集的 300 张图像进行数据增强，扩展到约 900 张，验证集不参与增强；
...
```mermaid
flowchart LR
    A[摄像头图像采集<br/>640 x 480] --> B[YOLOv8 INT8<br/>端侧目标检测]
    B --> C[检测结果解析<br/>类别 / 置信度 / 边界框]
    C --> D[目标候选筛选<br/>颜色 / 策略 / 面积]
    D --> E[视觉状态机<br/>目标搜索 / 抓取校验 / 安全区定位]
    E --> F[UART 数据帧<br/>坐标 / 面积 / 状态码]
    F --> G[机器人下位机]
    H[触摸屏参数配置<br/>队伍颜色 / N-P 策略 / OSD] --> E
```
...
程序通过 UART 向下位机发送固定长度的 11 字节数据帧。帧头和帧尾用于标记一帧数据的边界：

| 帧头（2 字节） | X 坐标（2 字节） | Y 坐标（2 字节） | 类型（1 字节） | 面积（2 字节） | 状态（1 字节） | 帧尾（1 字节） |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
| `X_H X_L` | 2 B | 目标中心横坐标，按百位和余数拆分 |
| `Y_H Y_L` | 2 B | 目标中心纵坐标，按百位和余数拆分 |
| `TYPE` | 1 B | `0x11`：找到目标；`0x22`：未找到目标；`0x00`：状态通知帧 |
| `AREA_H AREA_L` | 2 B | 缩放后的检测框面积 |
| `STATUS` | 1 B | `0x33`：抓取有效；`0x44`：抓取无效；`0x55`：继续搜索 |
| `CC` | 1 B | 帧尾，标记一帧数据结束 |

- `0xDD`：接收抓取完成信号并执行抓取条件判断；
- 接收 `0xCC`：表示到达安全区，恢复目标搜索。该字节在接收方向表示控制指令，在发送方向表示帧尾。
...
4. 需要打包安装时，使用 MaixVision 内置的应用打包/发布功能生成安装包，再安装到设备。

`app.yaml` 中列出了入口文件、图标和模型文件，打包时会将这些文件一起加入应用。默认串口为 `/dev/ttyS0`，波特率为 `115200`。

## 当前实测结果

以下结果均在 MaixCAM Pro 实际运行环境下测得，主指标统一使用 `dual_buff=True`：
...
- 训练数据、标注文件和训练脚本未随仓库公开，模型文件仅用于 MaixCAM 部署；
- 当前记录的是部署版本的推理吞吐率和完整运行 FPS；`96%` 为量化模型在数据集上的评估结果，不等同于现场识别准确率；
