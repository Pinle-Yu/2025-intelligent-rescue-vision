...
本项目面向双车对抗智能救援场景。视觉部分负责识别本方普通救援目标、核心救援目标、危险目标及安全区，并通过任务状态机完成目标选择与模式切换，为机器人将目标转运至本方安全区提供视觉信息。项目获得省一等奖。

负责视觉部分：

- 数据集制作：采集并标注 350 张图像，按 300/50 划分训练集和验证集，使用 Albumentations 将 300 张训练图像增强至 900 张。
- 训练 YOLOv8 目标检测模型，完成模型 INT8 量化与格式转换，并部署至 MaixCAM Pro，调用板载 NPU 实现端侧推理。
- 编写目标筛选、策略切换、视觉状态机和 UART 通信程序。

最终，训练阶段 `mAP50` 为 `99%`，INT8 量化模型在数据集上的 `mAP50` 为 `96%`，实际运行链路平均达到 `29.98 FPS`；决赛更换不同形状的物料后模型仍能完成稳定识别。
...
<table align="center">
  ...
  <tbody>
    <tr>
      <td>训练阶段 mAP50（mAP@0.5）</td>
      <td align="center"><code>99%</code></td>
    </tr>
    <tr>
      <td>INT8 量化模型在数据集上的 mAP50（mAP@0.5）</td>
